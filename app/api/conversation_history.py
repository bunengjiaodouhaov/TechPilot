...
async def list_workspace_conversations(
    ...
    return [
        ConversationResponse(
            ...
            turn_count=int(count or 0), created_at=c.created_at, updated_at=c.updated_at
        )
        for c, count in rows
    ]
...
async def create_workspace_conversation(
    ...
    return ConversationResponse(
        ...
        title=conversation.title, turn_count=0,
...
async def list_conversation_history(
    ...
    return [
        ConversationTurnResponse(
            id=t.id, workspace_id=t.workspace_id, conversation_id=t.conversation_id,
            role=t.role, text=t.text, refused=t.refused,
            citations=list(t.citations_json or []), created_at=t.created_at,
        )
        for t in turns
    ]
...
# Backward-compatible workspace history endpoints.
