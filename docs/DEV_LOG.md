...
<!-- P6_PRODUCTION_VALIDATION_DEVLOG_20260827_START -->
## P6 — Production Boundary Hardening / Production Validation

### Scope

P6 将主问题从“单请求功能能跑”切到 production boundary：authentication、authorization / tenant isolation、conversation concurrency、idempotency、transaction lifetime、DOCX real ingestion、production retrieval composition、provider/ingestion partial failure recovery。

### P6-1 Auth / isolation

- JWT/Cookie → `AuthPrincipal`；
- Workspace membership → `WorkspaceAuthorizer`；
- request `workspace_id` 不作为 permission proof；
- workspace list/answer/upload/conversation/product-memory 等 server-side gate；
- repository/cache owner identity isolation；
- PBKDF2-SHA256 + JWT HS256；production 禁止默认 secret。

Live workspace179：User A 对 User B workspace 的 `/answers` 和 `/documents/upload` 均 404；`GET /workspaces` 不可见179。真实 doc530 存在且 COMPLETED/70 chunks。对真实 doc530 的 unauthorized destructive delete 没有单独 live attack evidence，因此保留该证据缺口。

### P6-3 Conversation CAS

同 conversation 并发两次 `/answers`：一个200、一个409；409 detail=`conversation changed while the answer was generating; reload the latest history and retry`。最终只写成功请求的 user/assistant turns，stale 请求无 half-turn。PASS live。

### P6-4 Idempotency

`PROCESSING / COMPLETED / FAILED`，scope=user/workspace/operation/key/request_hash。same key + same request exact replay；same key + different payload 409；FAILED 可同请求 retry。Replay ~10ms。明确不声称 exactly-once：ingestion PG commit 与 endpoint idempotency completion 有 narrow crash window。

### DOCX production ingestion

新增 `.docx`：python-docx、paragraph/table body order、Heading1–6 hierarchy、structure-aware chunking、MIME/extension routing、UI upload；OOXML preflight 限制 entry count / uncompressed size / per-entry size / compression ratio，校验 required files，拒绝 bad/encrypted ZIP。图片/header-footer/nested complex layout 仍是 known limits。

### Exact DOCX semantic-gap failure

真实问题 `EnterpriseOps项目中权限校验是如何实现的？` 最初 production 安全拒答。PG/Qdrant/70 chunks 正常，关键证据为 chunk39/Q38 permission classes、chunk41/Q40 schema validation、chunk45/Q44 HITL approval hard boundary，因此定位为 retrieval/evidence admission semantic gap，不是 parser/indexing 故障。

### Reranker model selection

Strict 30-case same environment：

```text
old ms-marco MiniLM     Recall=.666667 MRR=.578889 regressions=5 inference~147ms
multilingual MiniLM     Recall=.866667 MRR=.740000 regressions=0 inference~253ms
BGE v2-m3               Recall=.866667 MRR=.766667 regressions=0 inference~2320ms
```

Production default 改为 `cross-encoder/mmarco-mMiniLMv2-L12-H384-v1`：与 BGE Recall/0-regression 相同，MRR只少`.026667`，inference约快9.2x。BGE offline quality 略高，不能反向描述。

### Rejected/diagnostic recovery experiments

- 12-case curated DOCX：multilingual baseline Recall@5 `.875`；manual flat `.9444`；manual multi `1.0`。manual expansions 只作为 architecture probe。
- Auto LLM decomposition：Recall@5 `.875`，无 coverage gain，query-generation mean ~1.36s；不做 mandatory rewrite。
- Grounded PRF：Recall@5 `.9444`、AnyHit@5 `1.0`，但额外 LLM hop 和 E2E latency 高，exact permission case 仍部分 gold；不做默认。
- Structural rerank expansion：CandidateRecall `.9167→.9722`、Recall@20 `.9167→.9444`，但 Recall@5/AnyHit 不提升；不做 unconditional expansion。

### Verifier-driven second chance

Production 只在 Verifier #1=`INSUFFICIENT` 时 recovery；`CONFLICTING` 直接拒答。第一版 top20 anchors→parent siblings→Verifier #2。

Trace 暴露 budget bug：first-pass 已覆盖“项目总览”support8 仍被优先扩展，MCP support3 被挤压。Fix：prefer novel parent sections + exclude existing anchors from additions。新 trace 后 chunk39/Q38、chunk41/Q40 成功进入 recovery。

随后确认 chunk45/Q44 真实属于下一 section `六、HITL / Reliability / Checkpoint`，不是 parser metadata bug。新增 boundary-aware selected-anchor `chunk_index ±1` bridge，保留真实 section provenance，nearest structural evidence 优先；无 EnterpriseOps/Q44/chunk45 hard-code。

### Final live semantic-gap E2E

Exact query 最终 `refused=false`、4 citations，覆盖 Q44 HITL approval、Q38 permission classes/default dangerous、Q39 dynamic discovery/permission metadata、Q40 schema validation。Q44 citation 保留真实 section。DOCX semantic-gap live E2E PASS。

准确权限口径：Gateway 负责 discovery/schema/permission class/approval metadata；真正 WRITE/DANGEROUS hard control 在 execution boundary，不能说全部 enforce 都发生在 Gateway。

### P6-5 Failure Recovery

补 provider timeout bounded exhaustion、`/answers` failure→idempotency FAILED→same-key retry、conversation provider failure no half-turn、CAS stale output no write、indexing compensation cleanup failure regression。

发现真实 fail-closed gap：如果 Qdrant compensation 自身失败，orphan point 可能指向 FAILED document；Answer-side `ChunkRepository` 原先未与 BM25 一样显式过滤 Document status。Fix：authoritative materialization 仅允许未删除且 `COMPLETED/PARTIAL`。
...
Targeted tests、full `pytest -q`、`git diff --check`：本地确认 PASS。

### Final Gate

P6 = **PASS WITH EXPLICIT KNOWN LIMITATIONS**。

已验证 auth/isolation、CAS、idempotency、short transaction boundary、DOCX、multilingual production reranker、semantic-gap bounded recovery、citation provenance、provider failure recovery、ingestion partial-failure searchable gate。

Known limits：不声称 exactly-once；Qdrant cleanup 仍可能 best-effort；P6 concurrency 不是高并发 load test；auth rate-limit/lockout/explicit CSRF 等有 backlog；DOCX multimodal/layout 不完整；真实 doc530 destructive unauthorized delete 没单独 live evidence；semantic recovery 不外推成 universal solution。

Next P7：throughput、P50/P95/P99、DB pool saturation、embedding/reranker/provider contention、retry amplification、backpressure/rate limiting、timeout/cancellation/cleanup、graceful degradation。
<!-- P6_PRODUCTION_VALIDATION_DEVLOG_20260827_END -->
