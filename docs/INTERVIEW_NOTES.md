...
<!-- P6_PRODUCTION_INTERVIEW_20260827_START -->
## P6 Production Validation — Interview Notes

> 本节 supersede 上方历史“当前生产链路 / 下一步”表述，但保留其历史真实性。

### Q：P6 解决的核心问题是什么？

P6 不再堆新 Agent feature，而是验证多用户、并发、重复请求和中途失败时，系统是否仍保持正确状态和 fail-closed boundary。范围包括 authentication / authorization、workspace isolation、conversation CAS、idempotency、transaction lifetime、DOCX、production reranker、semantic-gap recovery 和 partial-failure recovery。

### Q：Authentication 和 Authorization 怎么区分？

Authentication：`Bearer/Cookie -> JWT -> active User -> AuthPrincipal`。Authorization：`AuthPrincipal + workspace_id -> workspace_member -> WorkspaceAuthorizer`。request body 里的 `workspace_id` 只是资源标识，不是 permission proof。

### Q：为什么 Conversation 用 CAS，而不是长事务锁住 LLM？

一次回答包含 retrieval/reranker/verifier/LLM，可能秒级。把事务挂在慢网络调用上会放大 DB pool / lock 压力。当前先读取 conversation version V，释放事务，生成后只在 version 仍等于 V 时 append；版本变化返回 409。真实并发两请求得到 200/409，失败请求没有 half-turn。

### Q：Idempotency 为什么不能叫 exactly-once？

当前状态机是 `PROCESSING / COMPLETED / FAILED`；same key/same hash replay，same key/different hash conflict，FAILED 可同请求 retry。但 ingestion PostgreSQL business commit 与 endpoint idempotency completion 之间仍有 narrow crash window，因此准确说法是 request-level replay/deduplication，不是 distributed exactly-once。

### Q：为什么 production reranker 换成 multilingual MiniLM？

严格 30-case 同环境：旧 `ms-marco-MiniLM-L-6-v2` Recall `.666667`、MRR `.578889`、5 regressions；`mmarco-mMiniLMv2-L12-H384-v1` Recall `.866667`、MRR `.740000`、0 regression、inference ~253ms；BGE v2-m3 Recall `.866667`、MRR `.766667`、0 regression，但 inference ~2320ms。BGE 离线 MRR 略高，multilingual MiniLM 在同 Recall/0-regression 下约 9.2x 更快，所以选为 online default。

### Q：为什么换 reranker 后 EnterpriseOps 权限问题仍拒答？

因为 model choice 和 semantic-gap 是两类问题。关键 evidence 分散：Q38 permission class、Q40 schema validation、Q44 HITL approval；单轮 top-k 没拼完整复合机制，所以 Verifier 合理拒答。Parser/persistence/indexing 已确认正常。

### Q：为什么不直接把 top-k 调大或每次做 LLM query rewrite？

无条件加大 top-k 会增加 noise/cost；auto decomposition 实验没有 coverage gain且增加约 1.36s query-generation latency。Grounded PRF 更好但增加额外 LLM hop。Structural expansion 提升 CandidateRecall/Recall@20，却没有提升 Top5/AnyHit。实验结果都保留，但没有硬塞进 production。

### Q：最终 recovery 怎么实现？

只有 Verifier #1=`INSUFFICIENT` 时触发 bounded second chance；`CONFLICTING` 直接拒答。Recovery 使用 top-N anchors，优先 first-pass 尚未覆盖的 novel parent section，排除已有 anchors，再允许 selected anchor 的 `chunk_index ±1` 作为局部 cross-section boundary bridge；第二次 Verifier 仍不足就拒答。

### Q：Recovery 真实修了哪两个 bug？

第一，最初按 parent support count 排序，已经被 first-pass 覆盖的“项目总览”再次吞 recovery budget；修成 novel-parent priority。第二，Q44 实际属于下一真实 section，所以 same-parent expansion 永远拿不到；增加 bounded ±1 boundary bridge，并保留 Q44 自己的真实 section provenance。

### Q：最终 live E2E 证明了什么？

exact query `EnterpriseOps项目中权限校验是如何实现的？` 最终 `refused=false`，citations 覆盖 Q38、Q39、Q40、Q44。它证明这个真实 production semantic-gap case 已闭环，不证明所有跨章节问题 100% solved。

### Q：权限机制应该怎么准确讲？

不要说“所有权限都由 MCP Gateway 自己 enforce”。准确表述：Gateway 负责 tool discovery、schema validation、permission classification 和 approval metadata；真正 WRITE/DANGEROUS 的 hard control 在工具执行边界，approval 未通过前不产生 side effect。Prompt 里的“先问我”只是软约束。

### Q：P6-5 failure recovery 的关键修复是什么？

除了 provider bounded retry、idempotency FAILED→retry、conversation 无 half-turn，还发现一个 fail-closed gap：若 Qdrant partial-index compensation 自身失败，orphan vector 可能指向 FAILED document。修复后 Answer-side `ChunkRepository` 与 BM25 一样，只 materialize 未删除且 `COMPLETED/PARTIAL` 文档，所以 orphan index 不能升级为 authoritative evidence。

### Q：P6 最终 Gate 怎么讲？

**PASS WITH EXPLICIT KNOWN LIMITATIONS**。已 live 证明 auth isolation、CAS、idempotency、DOCX exact semantic-gap E2E、failure-recovery DB probe；但没有做 100/1000 concurrency load test，不声称 exactly-once，不声称完整 IAM hardening，不声称 DOCX multimodal，不声称所有 destructive endpoint 都逐条 live attack。

### Q：下一步为什么是 P7 高并发？

单请求正确性已有足够证据；下一类 production risk 是 DB pool、embedding、BM25 request work、Qdrant、CrossEncoder、provider concurrency 和 retry amplification 的资源竞争。P7 要测 throughput、P50/P95/P99、saturation、backpressure、rate limit、timeout/cancellation/cleanup 和 graceful degradation。

### P6 面试红线

- 不说 exactly-once / distributed transaction；
- 不说 BGE 质量更差，BGE 离线 MRR 实际略高；
- 不说所有 destructive endpoint 已逐条 live 验证；
- 不说 P6 已做高并发压力测试；
- 不说 DOCX 图片/复杂 layout 都能理解；
- 不说 recovery universal solve semantic-gap；
- 不说 P5 Job Intelligence Product Gate PASS。
<!-- P6_PRODUCTION_INTERVIEW_20260827_END -->
