...
<!-- P6_PRODUCTION_VALIDATION_20260827_START -->
## P6 Production Boundary Hardening / Production Validation — Current Superseding Status

> 本节 supersede 文档上方所有历史“当前阶段 / 下一步 / production retrieval”表述，但不改写它们当时的历史真实性。

### 当前结论

**P6 = PASS WITH EXPLICIT KNOWN LIMITATIONS**。

P5 已冻结为 real-business prototype；当前下一阶段是 P7 high-concurrency / load / service-level reliability validation。

### P6 已验证

- JWT / Cookie authentication + active user validation；
- workspace membership authorization，request `workspace_id` 不作为权限依据；
- cross-user workspace list / answer / upload isolation live PASS；
- conversation optimistic CAS：并发两请求 200/409，stale request 无 half-turn；
- idempotency exact replay / same-key different-payload conflict / FAILED→retry；
- slow provider path 前释放 DB transaction；
- DOCX paragraph/table/heading ingestion + OOXML ZIP preflight；
- production retrieval 已从历史 Dense-only 状态升级为 Dense + BM25 + RRF + CrossEncoder；
- production reranker 选为 `cross-encoder/mmarco-mMiniLMv2-L12-H384-v1`；
- verifier-driven bounded semantic-gap recovery；
- novel parent-section priority + anchor dedupe + boundary-aware ±1 bridge；
- exact EnterpriseOps permission query live `/answers`: `refused=false` + grounded citations；
- provider timeout bounded retry / failure recovery；
- ingestion post-commit indexing failure → FAILED + non-searchable live DB probe；
- full regression + `git diff --check`：本地确认 PASS。

### Strict P2 reranker comparison

| Model | Recall@5 | MRR@5 | Regressions | Rerank inference mean |
| --- | ---: | ---: | ---: | ---: |
...
选择 multilingual MiniLM 是 production quality/latency trade-off；不能说 BGE 质量更差。

### Semantic-gap live evidence

原 query：`EnterpriseOps项目中权限校验是如何实现的？`

最初单轮链路安全拒答。最终 recovery 后真实 citations 覆盖：

- Q38：READ / WRITE / DANGEROUS + unknown default dangerous；
- Q39：dynamic tool discovery / permission metadata；
- Q40：server call 前 schema validation；
- Q44：WRITE/DANGEROUS approval hard boundary，批准前不执行。

Q44 位于下一真实 section；boundary bridge 保留真实 provenance，不伪造章节。
...
`ChunkRepository` authoritative load 与 BM25 searchable contract 对齐，只接受未删除且 `COMPLETED/PARTIAL` Document。即使 compensation cleanup failure 留下 orphan Qdrant point，也不能升级为 Answer evidence。

### Known limitations / red lines

- 不声称 exactly-once；ingestion PG commit 与 idempotency completion 有 narrow crash window；
- Qdrant cleanup 仍可能 best-effort；correctness 依赖 PostgreSQL authoritative fail-closed gate；
- P6 CAS 并发验证不是 100/1000 并发压测；
- open registration / rate-limit / lockout / explicit CSRF 等仍是 auth hardening backlog；
- DOCX 图片 / 复杂 layout 尚未 multimodal 理解；
- 对真实 document530 的 unauthorized destructive delete 没有单独 live attack evidence；
- semantic recovery 已通过真实复合 case，但不外推成所有跨章节问题 100% solved。

### 下一步

P7：throughput、P50/P95/P99、DB pool saturation、reranker/provider contention、retry amplification、backpressure、rate limit、timeout budget、cancellation/cleanup、graceful degradation。
<!-- P6_PRODUCTION_VALIDATION_20260827_END -->
